...
Compiled under R version 4.0.3 (2020-10-10)
...
Legendre, L. J., and J. A. Clarke. 2020. Shifts in eggshell thickness are related to changes in locomotor ecology and body size in dinosaurs.
...
Modnames = paste(c("BM", "OU", "Lambda", "EB", "OLS", "BM2", "OU2", "Lambda2", "EB2", "OLS2"), sep = " ")
aictab(cand.set = Cand.models, modnames = Modnames, sort = T)
...
```{r}
summary(Lambda)
Lambdanoint<-gls(log(Eggshell_thickness)~log(Egg_mass)+Flight, data=dataflight, correlation=corPagel(0.1, phy=treeflight), weights=varFixed(~Wf), method="ML")
...
- Boxplots for each of those on rellative eggshell thickness – i.e. ratio (eggshell thickness/egg mass)
```{r}
...
*Flyers and non-flyers are clearly separated*, showing an influence of flight on both variables. However, since we know that the influence of flight on eggshell thickness is indirect through that of egg mass, it is possible that the difference in eggshell thickness between flyers and non flyers is entirely driven by a difference in egg mass (reflecting a difference in body mass).
...
- *The difference between flyers and non-flyers for eggshell thickness is most likely driven by body mass* (egg mass being a proxy for it), since after correction for body mass that difference is *not significant*.
...
# In specific clades
shifts<-search.shift(ratesDcov, status.type= "clade", node=c(119,122,137,141,153,160,166,167,178,194,207), cov=EM, foldername=tempdir())
shifts
...
trend<-search.trend(ratesDcop, y=ET, nsim=100,
             node=c(119,122,137,141,153,160,166,167,178,194,207),
             x1=x1mass,
             foldername=tempdir())
trend
```
...
overfitRR(ratesDcop, y=ET, trend.args=list(node=c(178,194)),
          x1=x1mass, nsim=100)
overfitRR(ratesDcop, y=ET, trend.args=list(node=c(207)),
          x1=x1mass, nsim=100)
```
...
# In specific clades
shifts<-search.shift(ratesDcov, status.type= "clade", node=c(118,121,136,141,153,158,160,161,172,188,189,201), cov=Flight, foldername=tempdir())
shifts
...
trend<-search.trend(ratesDcop, y=EMn, nsim=100,
             node=c(118,121,136,141,153,158,160,161,172,188,189,201),
             x1=x1flight,
             foldername=tempdir())
```
...
```{r}
# for search.shift
SSnode<-search.shift(ratesDcov, status.type= "clade", node=c(136,141,153,158,160,161,172,188,189,201), cov=Flight,foldername=tempdir())
SSnames<-rownames(SSnode$single.clades)
...
# search.shift
shifts<-search.shift(RREM, status.type= "clade", node=c(118,121,136,141,153,158,160,161,171,188,189,201), foldername=tempdir())
shifts

# search.trend
search.trend(RREM, y=EM, nsim=100,
             node=c(118,121,136,141,153,158,160,161,171,188,189,201),
             foldername=tempdir())
...
## for search.trend
STnode<-search.trend(RREM, y=EM, node=c(118,121,153,158,160,161,189,201),
...
overfitRR(RREM, y=EM, trend.args=list(node=c(201)), nsim=100)
```
...
# In specific clades
shifts<-search.shift(ratesDcov, status.type= "clade", node=c(118,121,136,141,153,158,160,161,172,188,201), cov=Flight, foldername=tempdir())
shifts
...
trend<-search.trend(ratesDcop, y=ETMn, nsim=100,
             node=c(118,121,136,141,153,158,160,161,172,188,201),
...
```{r}
# for search.trend
STnode<-search.trend(ratesDcop, y=ETMn, node=c(118,121,136,141,153,158,160,161,172,188,201),
             clus=0.5,foldername=tempdir(), x1=x1flight, ConfInt=F)
overfitRR(ratesDcop, y=ETMn, trend.args=list(node=c(118)), x1=x1flight,
          nsim=100)
...
- Difference between the two equations (Hoyt, 1979; Packard and DeMarco, 1991) for egg mass estimation
```{r}
equatable<-read.table("Test_equation.txt", header=T)
t.test(equatable$A,equatable$B)
```
...
- New regressions for each of them
```{r}
dataD<-dataD[-c(25:27,97),]
...
```{r}
# Data and trees
datapar<-subset(dataD, dataD$Paravian=="Yes")
datanopar<-subset(dataD, dataD$Paravian=="No")
...
slopereg2<-gls(log(Eggshell_thickness)~log(Egg_mass)*Paravian, data=dataD,
               correlation=corPagel(0, phy=treeD, fixed=T),
